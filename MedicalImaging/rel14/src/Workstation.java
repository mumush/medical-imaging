...
public class Workstation implements Observer {
   ...
   public void update(Observable obs, Object obj) {
	...
	//if previous image button was pushed//
	else if( obj.equals("previous image") ) {
		//checks current display state and generates accordingly//
		if(study.getDisplayState()==1){
			String imgPath = study.getImageList().get(study.getCurrentImage()).getAddress();
		...
		System.out.println("something was saved");
	}
	
	else{
		localStudy newStudy= (localStudy) obj;
		newStudy.addObserver(this);
		control.addStudy(newStudy);
		System.out.println("well... we're not dead");
...
public Workstation(Controller ctrl) {
	...
	//create new JMenuItem for the Preferences menu
	defaultItem = new JMenuItem("Set as Default Study");
	//add the aboutItem to the helpMenu
	prefsMenu.add(defaultItem);
	...
	openItem.addActionListener(ctrlListener);
  	...
  	defaultItem.addActionListener(ctrlListener);
